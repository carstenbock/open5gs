...
void upf_pfcp_state_associated(ogs_fsm_t *s, upf_event_t *e)
{
    ...
    case UPF_EVT_N4_NO_HEARTBEAT:
         ...
         * The SMF/PGW-C became unreachable. In Kubernetes the CP normally
         * restarts on a NEW pod IP, which the passive UP sees as a brand-new
         * PFCP node, so the Recovery-Time-Stamp restart detection never fires
         * for THIS (old) node. Release the sessions it anchored, otherwise the
         * user plane keeps state for orphaned bearers the restarted CP no
         * longer knows about and black-holes their downlink until a manual UPF
         * restart. This mirrors the active side (smf reselect_upf on
